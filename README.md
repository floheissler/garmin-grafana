...
✅ **Multi-device accounts** — if your Garmin account has more than one device syncing (e.g. a watch + a blood-pressure cuff + a chest strap), you should explicitly pin the watch's identity via the `GARMIN_DEVICENAME` and `GARMIN_DEVICEID` env vars in `compose.yml`. Without the pin, the script auto-detects the device by reading `lastUsedDeviceName` from Garmin every 5-min cycle — when a non-watch device syncs (e.g. you take a BP reading), the next cycle then writes the watch's daily/sleep/HRV/intraday data with the *non-watch* device's name as the InfluxDB `Device` tag, creating phantom series that never overwrite and inflate trend counts. The pinned name should match what Garmin Connect displays for your watch (e.g. `fenix 8 - 51mm, AMOLED`). If you also have a blood-pressure cuff, set `GARMIN_BPM_DEVICENAME` to a friendly name (e.g. `Index™ BPM`) — Garmin's API only returns `sourceType="DEVICE"` for cuff readings, so without this env var your `BloodPressure` rows would all be tagged with the literal string "DEVICE".
